...
RTC_DATA_ATTR struct WeatherGSR final {
   ...
   struct Weatherinfo {
     ...
     uint8_t Clouds;       /* "clouds" from api.openweathermap.org */
     uint16_t Pressure;    /* "pressure" from api.openweathermap.org */
     time_t SunRise;       /* "sunrise" from api.openweathermap.org */
     time_t SunSet;        /* "sunset" from api.openweathermap.org */
     uint32_t Visibility;  /* "visibility" from api.openweathermap.org */
...
void WatchyGSR::StartWeb(){
    ...
    server.on("/settings", HTTP_POST, [=](){
        if (server.argName(0) == "settings") { StoreSettings(server.arg(0)); if (Options.WatchFaceStyle > WatchStyles.Count - 1) Options.WatchFaceStyle = 0; RecordSettings(); WatchFaceStart(Options.WatchFaceStyle); }
        server.sendHeader("Connection", "close");
      ...
      if (GuiMode == GSR_MENUON){     // Up Button [SW3]
          ...
          if (Menu.Item == GSR_MENU_STEPS && Menu.SubItem > 0){
              ...
              if (Menu.Style == GSR_MENU_INOPTIONS){
                  ...
                  if (Menu.Item == GSR_MENU_WEAT && !GetWantWeather()) Menu.Item = roller(Menu.Item - 1, GSR_MENU_STYL, (NTPData.State > 0 || WatchyAPOn || OTAUpdate || Battery.Last < Battery.MinLevel) ? GSR_MENU_TRBL : GSR_MENU_OTAM);
...
void WatchyGSR::AskForWiFi(){ SetAskWiFi(true); GSRWiFi.Requests++; }
void WatchyGSR::endWiFi(){
    if (!GetAskWiFi() && (GSRWiFi.Powered || GSRWiFi.Requests > 0)) GSRWiFi.Requests = 1;
...
void WatchyGSR::initZeros(){
    ...
    WatchStyles.Count = 0;
    BasicWatchStyles = -1;
    for (I = 0; I < GSR_MaxStyles; I++) { WatchStyles.Options[I]=0; WatchStyles.AddOn[I] = nullptr; BootAddOns.Inits[I] = nullptr; if (I<32) WeatherData.APIKey[I] = 0; } BootAddOns.Count = 0;
...
void WatchyGSR::StartWeather(){
  if (WeatherData.State == 0 && !WatchTime.DeadRTC && NTPData.State < 2 && String(WeatherData.APIKey) != "" && GSRHandle == NULL && GetWantWeather() && Battery.Last > Battery.LowLevel){
    WeatherData.State = 1;
    WeatherData.Pause = 2;
    AskForWiFi();
  }
    ...
    case 5:{
      ...
      if (GetWebReady()) {
        ...
        if (E == 401){ for(int I = 0; I < 32; I++) WeatherData.APIKey[I] = 0; Options.NeedsSaving = true; }
            ...
            S = CleanString(JSON.stringify(root["clouds"]["all"]));
            WeatherData.Weather.Clouds = uint8_t(S.toInt());
            S = CleanString(JSON.stringify(root["main"]["pressure"]));
            ...
            S = CleanString(JSON.stringify(root["sys"]["sunrise"]));
            WeatherData.Weather.SunRise = time_t(S.toInt());
            S = CleanString(JSON.stringify(root["sys"]["sunset"]));
            WeatherData.Weather.SunSet = time_t(S.toInt());
            S = CleanString(JSON.stringify(root["visibility"]));
...
unsigned long Stay = millis() + 4000 + webTimeout;
    while (Good && millis() < Stay){
        ...
        if (!Sent && Good) { Sent = true; HTTP.setConnectTimeout(webTimeout); HTTP.begin(WiFiC, GSRWebData.webURL); }
        if (GSRWebData.Response == HTTP_CODE_OK) {
            GSRWebData.Data= HTTP.getString();
            GSRWebData.Ready = true;
            Good = false;
        }
        if (Good) { I = HTTP.GET(); if (I != 0) { GSRWebData.Response = I; if (I != HTTP_CODE_OK) Good = false; } }
